...
export default async function handle(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (req.method === 'PUT' || req.method === 'POST') {
    try {
      ...
      let notiTime: Date | null = new Date();

      if (latest) {
        // If status is the same, check cooldown (e.g. 5 minutes)
        if (latest.locat_status === calculatedStatus) {
          const lastNoti = moment(latest.locat_noti_time);
          const now = moment();
          const diffMinutes = now.diff(lastNoti, 'minutes');
          if (diffMinutes < 5) {
            shouldNotify = false;
            notiTime = latest.locat_noti_time; // Keep original notification time
          }
        }
        // If status changed, notify immediately (shouldNotify = true by default)
      }
      ...
      // แจ้งเตือน (เมื่อ shouldNotify เป็น true เท่านั้น)
      if (shouldNotify) {
